...
$$\hat{\mu}_N = \frac{1}{N}\sum_{i=1}^{N} X_i \;\xrightarrow{\text{a.s.}}\; \mathbb{E}[X]$$

**Unbiased sample variance.** Dispersion estimator with Bessel's correction:

$$S^2 = \frac{1}{N-1}\sum_{i=1}^{N}\bigl(X_i - \hat{\mu}_N\bigr)^2$$

**Standard error of the mean.** Source of the famous $\mathcal{O}(N^{-1/2})$ convergence rate:

$$\mathrm{SE}(\hat{\mu}_N) = \frac{S}{\sqrt{N}}$$

**Normal-approximation confidence interval** at confidence level $1-\alpha$:

$$\mathrm{CI}_{1-\alpha} = \left[\,\hat{\mu}_N - z_{1-\alpha/2}\,\mathrm{SE},\;\;\hat{\mu}_N + z_{1-\alpha/2}\,\mathrm{SE}\,\right]$$

**Monte Carlo integration.** For an integrable function $f$ on a domain $\Omega \subset \mathbb{R}^d$ with volume $V(\Omega)$:

$$I \;=\; \int_{\Omega} f(\mathbf{x})\,d\mathbf{x} \;\approx\; \frac{V(\Omega)}{N}\sum_{i=1}^{N} f(\mathbf{X}_i), \qquad \mathbf{X}_i \sim \mathcal{U}(\Omega)$$
...
$$\hat{I}_{\mathrm{IS}} \;=\; \frac{1}{N}\sum_{i=1}^{N} f(X_i)\,\frac{p(X_i)}{q(X_i)}, \qquad X_i \sim q$$

**Self-normalized importance sampling**, valid when $p$ and $q$ are known only up to a normalizing constant:

$$\hat{I}_{\mathrm{SNIS}} \;=\; \frac{\sum_{i=1}^{N} f(X_i)\,w_i}{\sum_{i=1}^{N} w_i}, \qquad w_i = \frac{p(X_i)}{q(X_i)}$$
...
$$H(\mathbf{x}, \mathbf{p}) \;=\; -\log p(\mathbf{x}) \;+\; \tfrac{1}{2}\,\mathbf{p}^{\mathsf{T}} M^{-1} \mathbf{p}$$
...
$$\hat{\mu}_n = \hat{\mu}_{n-1} + \frac{X_n - \hat{\mu}_{n-1}}{n}, \qquad M_n = M_{n-1} + (X_n - \hat{\mu}_{n-1})(X_n - \hat{\mu}_n), \qquad S_n^2 = \frac{M_n}{n-1}$$
...
**Bayesian Quadrature** (O'Hagan, 1991). Treat the integrand as a Gaussian process. Under an RBF kernel paired with a Gaussian measure $\pi = \mathcal{N}(b, B)$, the posterior over $I = \int f(\mathbf{x})\pi(\mathbf{x})\,d\mathbf{x}$ has closed form:

$$\mathbb{E}[\,I \mid \mathcal{D}\,] = \mathbf{z}^{\mathsf{T}} K^{-1} \mathbf{y}, \qquad \mathrm{Var}[\,I \mid \mathcal{D}\,] = \iint k(\mathbf{x},\mathbf{x}')\,\pi(\mathbf{x})\pi(\mathbf{x}')\,d\mathbf{x}\,d\mathbf{x}' \;-\; \mathbf{z}^{\mathsf{T}} K^{-1} \mathbf{z}$$
